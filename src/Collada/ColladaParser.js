...
/**
 *	@constructor ColladaParser
 *	
 */
GlobWeb.ColladaParser = (function()
{
	/**
	...
	var parseTexture = function(effect,node)
	{
		...
		return new GlobWeb.SceneGraph.Texture( baseURI + findElementByTag( image, "init_from" ).textContent.trim() );
	}
	...
	{
		var material = new GlobWeb.SceneGraph.Material();
	...
	var parsePolygons = function(node,colladaGeometry)
	{
		...
		var mesh = new GlobWeb.SceneGraph.Mesh();
	...
	{
		var node = new GlobWeb.SceneGraph.Node();
		...
		while ( child )
		{
			...
			case "instance_geometry":
				if ( geometries.hasOwnProperty( child.getAttribute("url") ) )
				{
					...
					{
						var geometry = new GlobWeb.SceneGraph.Geometry();
	...
	var parseLibraryVisualScenes = function(library)
	{
		...
		var root = new GlobWeb.SceneGraph.Node();
		sceneGraph = new GlobWeb.SceneGraph(root);
